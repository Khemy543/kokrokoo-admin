...
import {
  ...
  class RejectedPo extends React.Component{
    ...
    axios.get(`${domain}/api/admin/fetch/rejected/pos?page=${pageNumber}`,
    {headers:{ 'Authorization':`Bearer ${user}`}})
    .then(res=>{
        console.log(res.data)
        ...
        {this.state.po.length<=0?
          <Row>
              <Col style={{textAlign:"center"}}>
                  <h4>You Have No Rejected Purchase Order</h4>
              </Col>
          </Row> 
          :
          <Container className=" mt--8" fluid>
          ...
          <p style={{fontSize:"13px", fontWeight:500}}>View Rejected Purchase Order.</p>
          ...
          {this.state.po.map((value,key)=>(
              ...
              <td>
              <Button color="info" style={{borderRadius:"100%", padding:"2px 5px 2px 5px"}} onClick={()=>this.props.history.push("/admin/view-rejected-pos",{cart_id:value.cart_id})}><i className="fa fa-eye"/></Button>
