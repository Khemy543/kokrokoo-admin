...
import {
  ...
  class PendingPo extends React.Component{
    ...
    axios.get(`${domain}/api/admin/fetch/pending/pos?page=${pageNumber}`,
    {headers:{ 'Authorization':`Bearer ${user}`}})
    .then(res=>{
        console.log(res.data)
        ...
        {this.state.po.length<=0?
          <Row>
              <Col style={{textAlign:"center"}}>
                  <h4>You Have No Pending Purchase Order</h4>
              </Col>
          </Row>
          :
          <Container className=" mt--8" fluid>
          ...
          <p style={{fontSize:"13px", fontWeight:500}}>View Pending Purchase Order.</p>
          ...
          {this.state.po.map((value,key)=>(
              ...
              onClick={()=>this.props.history.push("/admin/view-pending-pos",{id:value.id,cart_id:value.cart_id})}
